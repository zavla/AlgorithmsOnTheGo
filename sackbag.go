...
func sackbag[T ~int](W T, m []T, c []T) (T, []T) {
	...
	//т.к. индекс kg сравнивается с реальным весом в кг то его надо начать с 1, а не 0.
	for kg := T(1); kg <= W; kg++ { //для каждого кг, т.е. для всех возможных разных мешков по вместимости.
