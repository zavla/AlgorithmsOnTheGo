...
//масса 		m 1 4 6 3
// ---------
//на входе массив весов предметов m и массис с стоимости предметов.
...
func sackback(W int, m, c []int) (int, []int) {
	...
	//на десятом грузике мы уже в таблице видим что если мы его берем то заполненность рюкзака чтобы его взять
	//оптимальная такая-то. Добавим десятый предмет - получим выгоду такую-то. Проверим что она не меньше
	...
	for kg := 1; kg <= W; kg++ { //для каждого кг, т.е. для каждого свободного места в мешке
		for j := 0; j < N; j++ { //для каждого грузика, j это номер грузика
			...
			if kg >= m[j] { //если рассматриваем мешок больший чем вес грузика
